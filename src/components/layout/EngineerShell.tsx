...
import { engineerNav } from "@/components/layout/nav";
  ...
  return (
    <div className="min-h-dvh bg-canvas">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 flex-col bg-navy text-white lg:flex">
        ...
        <div className="border-t border-white/10 p-4 text-sm text-blue-100/70">
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-20 bg-navy text-white">
          <div className="flex items-center justify-between px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))] lg:px-8">
            <button className="rounded-2xl p-2 hover:bg-white/10 lg:hidden" onClick={() => setOpen((v) => !v)}>
              <Menu className="h-5 w-5" />
            </button>
            <div className="lg:hidden">
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-blue-200">Field App</p>
              <p className="text-sm font-bold">{APP_NAME}</p>
            </div>
            <div className="hidden lg:block">
              <p className="text-sm font-semibold">Field operations workspace</p>
              <p className="text-xs text-blue-100/70">Tickets and attendance</p>
            </div>
            <div className="ml-auto flex items-center gap-3">
              <div className="hidden text-right sm:block">
                <p className="text-sm font-semibold">{user.name}</p>
                <p className="text-xs text-blue-100/70">Field Engineer</p>
              </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-line bg-white/95 backdrop-blur lg:hidden">
        <div className="grid grid-cols-4 pb-[max(0.45rem,env(safe-area-inset-bottom))]">
          {engineerNav.map((item) => {
            const active = pathname === item.href;
            return (
              <Link
